...
prop(Context.prototype, '__helpers__', {
  asset_data_uri:   {name: 'asset_data_uri', func: Context.assetDataUri, opts: {kind: 'generic', fname: 'assetDataUri'}},
  asset_path:       {name: 'asset_path', func: Context.assetPath, opts: {kind: 'generic', fname: 'assetPath'}}
});
...
Context.prototype.evaluate = function (pathname, options, callback) {
  ...
  _.each(this.__helpers__, function (helper, name) {
    console.log("ev name:", name, " helper:", helper);
...
function define_helpers_registrator(Klass) {
  Klass.registerHelper = function (name, func, opts) {
    ...
    opts = opts || {};
    _.defaults(opts, {kind: 'generic', fname: name});
    console.log("registering helper ", name, " opts:", opts);
    var helper = {
      name: name,
      func: func,
      opts: opts
    ...
    if (_.isString(name)) {
      ...
      Klass.prototype.__helpers__[name] = helper;
      return;
    }
