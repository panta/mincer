...
// Hash of stylus helpers available as locals in the renderers.
// For each helper, `this` will be the context stylus is passing to helpers.
//
// See Context.registerStylusHelper()
prop(Context.prototype, '__stylus_helpers__', {
  asset_data_uri:   'assetDataUri',
  asset_path:       'assetPath'
});
...
['image', 'video', 'audio', 'font', 'javascript', 'stylesheet'].forEach(function (type) {
  Context.prototype.__helpers__[type + '_path'] = type + 'Path';
  Context.prototype[type + 'Path'] = function (pathname) {
...
Context.prototype.evaluate = function (pathname, options, callback) {
  ...
  _.each(this.__stylus_helpers__, function (func, name) {
    locals[name] = self[func];  // no wrapper here: stylus helpers need the stylus context
  });
...
function define_helpers_registrator(Klass) {
  ...
  Klass.registerStylusHelper = function (name, func) {
    // Scenario: registerStylusHelper('foo', foo_helper);
    if (_.isString(name)) {
      var stylus_helper_name = '_stylus_helper_' + name;
      Klass.prototype[stylus_helper_name] = func;
      Klass.prototype.__stylus_helpers__[name] = stylus_helper_name;
      return;
    }

    // Scenario: registerStylusHelper({ foo: foo_helper, ... });
    _.each(name, function (func, name) {
      Klass.registerStylusHelper(name, func);
    });
  };
}
...
getter(Context, 'subclass', function () {
  ...
  prop(Klass.prototype, '__stylus_helpers__', _.clone(Context.prototype.__stylus_helpers__));
